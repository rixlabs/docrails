...
Dopo aver letto quetsa guida saprai:

* Come seguire il flusso di una richiesta attraverso il controller.
* Come limitare i parametri passati al tuo controller.
* Perché e come salvare i dati nella sessione o nei cookies.
* Come lavorare con i filtri per eseguire del codice durante il trattamento di una richiesta.
* Come utilizzare l'autenticazione HTTP integrata in Action Controller.
* Come creare uno stream di dati direttamente sul browser dell'utente.
* Come filtrare i dati sensibili in modo che non appaiano nei log dell'applicazione.
* Come trattare le eccezioni che potrebbero venir sollevate durante il trattamento delle richieste.
...
Action Controller é la C in MVC. Dopo che la procedura di routing ha determinato quale controller utilizzare per una richiesta, il tuo controller diventa responsabile di portare a termine la richiesta e produrre l'output appropriato. Fortunatamente, Action Controller fa la gran parte del lavoro sporco per te e utilizza delle convenzioni intelligenti per farlo nel modo più semplice possibile.

Per le classiche [RESTful](http://it.wikipedia.org/wiki/Representational_State_Transfer) applications, il controller riceverà la richiesta (questa sarà invisibile allo sviluppatore), estrarrà o salveré i dati da un modello e utilizzerà una view per creare l'output HTML. Se il tuo controller fare qualcosa di leggermente differente, non é un problema, questa é soltanto il modo più comune di lavorare per un controller.

Si può pensare al controller come un attore che agisce tra i modelli e le views. Mette a disposizione delle views i dati del modello in modo che possano essere visualizzati dall'utente e salva o aggiorni i dati provenienti dall'utente nel modello. 

NOTA: Per maggiori dettagli riguardo al processo di routing, visita [Rails Routing from the Outside In](routing.html).
